                        ...
                        loc.materials.length > 0 ? (
                            loc.materials.map((mat, matIdx) => (
                                React.createElement("tr", { key: mat.id, className: "border-t border-zinc-700 hover:bg-zinc-700/50" },
                                        ...
                                        React.createElement("td", { rowSpan: loc.materials.length, className: "p-3 font-semibold text-yellow-300 align-top" },
                                             isEditing ? (
                                                React.createElement("div", { className: "flex flex-col items-start gap-2" },
                                                    ...
                                                    React.createElement("button", { onClick: () => handleAddMaterial(locIdx), className: "flex items-center gap-1 text-xs px-2 py-1 bg-green-600 hover:bg-green-500 rounded text-white" }, React.createElement(PlusCircleIcon, { className: "w-4 h-4" }), " Añadir Material")
                                                )
                                            ) : loc.name
                                        )
                                    ),
                                    isEditing ? (
                                        React.createElement(React.Fragment, null,
                                            React.createElement("td", null, React.createElement("input", { value: mat.name, onChange: e => handleMaterialChange(locIdx, matIdx, 'name', e.target.value), className: "w-full bg-zinc-700 rounded p-1" })),
                                            React.createElement("td", { className: "text-center" }, React.createElement("input", { type: "number", value: mat.quantity, onChange: e => handleMaterialChange(locIdx, matIdx, 'quantity', e.target.value), className: "w-20 bg-zinc-700 rounded p-1 text-center" })),
                                            React.createElement("td", null,
                                                React.createElement("select", { value: mat.condition, onChange: e => handleMaterialChange(locIdx, matIdx, 'condition', e.target.value), className: `w-full border-zinc-600 rounded p-1 font-semibold ${getConditionColor(mat.condition)}` },
                                            ...
                                            React.createElement("td", null, React.createElement("input", { value: mat.location || '', onChange: e => handleMaterialChange(locIdx, matIdx, 'location', e.target.value), className: "w-full bg-zinc-700 rounded p-1" })),
                                            React.createElement("td", null, React.createElement("button", { onClick: () => handleRemoveMaterial(locIdx, matIdx), className: "p-1 text-red-400 hover:text-red-300" }, React.createElement(TrashIcon, { className: "w-5 h-5" })))
                                        )
                                    ) : (
                                        React.createElement(React.Fragment, null,
                            ...
                            React.createElement("tr", { key: loc.name, className: "border-t border-zinc-700" },
                                ...
                                React.createElement("td", { colSpan: isEditing ? 5 : 4, className: "p-3 text-center text-zinc-500 italic" }, "No hay materiales registrados.")
                            )
                        )
                    ))
                )
